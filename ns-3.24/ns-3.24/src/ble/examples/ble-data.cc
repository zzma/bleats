...
/*
 * Try to send data end-to-end through a BleMac <-> BlePhy <->
 * SpectrumChannel <-> BlePhy <-> BleMac chain
 *
...
{
  NS_LOG_UNCOND ("BleMcpsDataConfirmStatus = " << params.m_status);
}

static void StateChangeNotification (std::string context, Time now, BlePhyEnumeration oldState, BlePhyEnumeration newState)
{
  NS_LOG_UNCOND (context << " state change at " << now.GetSeconds ()
                         << " from " << BleHelper::BlePhyEnumerationPrinter (oldState)
                         << " to " << BleHelper::BlePhyEnumerationPrinter (newState));
}
...
int main (int argc, char *argv[])
{
  ...
  // Tracing
  bleHelper.EnablePcapAll (std::string ("ble-data"), true);
  AsciiTraceHelper ascii;
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream ("ble-data.tr");
  bleHelper.EnableAsciiAll (stream);
