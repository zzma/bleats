...
NS_LOG_COMPONENT_DEFINE ("BleErrorModelPlot");
...
int main (int argc, char *argv[])
{
  ...
  Ptr<BleErrorModel>  bleError = CreateObject<BleErrorModel> ();
  ...
  for (double snr = minSnr; snr <= maxSnr; snr += increment)
    {
      double ber = 1.0 - bleError->GetChunkSuccessRate (pow (10.0,snr / 10.0), 1);
