...
NS_LOG_COMPONENT_DEFINE ("BleCsmaCa");

NS_OBJECT_ENSURE_REGISTERED (BleCsmaCa);

TypeId
BleCsmaCa::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BleCsmaCa")
    .SetParent<Object> ()
    .SetGroupName ("Ble")
    .AddConstructor<BleCsmaCa> ()
  ;
  return tid;
}
...
BleCsmaCa::DoDispose ()
{
  m_bleMacStateCallback = MakeNullCallback< void, BleMacState> ();
  Cancel ();
  m_mac = 0;
}

void
BleCsmaCa::SetMac (Ptr<BleMac> mac)
{
  m_mac = mac;
}

Ptr<BleMac>
BleCsmaCa::GetMac (void) const
{
  return m_mac;
}

void
BleCsmaCa::SetSlottedCsmaCa (void)
{
...
void
BleCsmaCa::SetUnSlottedCsmaCa (void)
{
  NS_LOG_FUNCTION (this);
  m_isSlotted = false;
}
...
bool
BleCsmaCa::IsSlottedCsmaCa (void) const
{
  NS_LOG_FUNCTION (this);
  return (m_isSlotted);
}
...
bool
BleCsmaCa::IsUnSlottedCsmaCa (void) const
{
  NS_LOG_FUNCTION (this);
  return (!m_isSlotted);
}
...
void
BleCsmaCa::SetMacMinBE (uint8_t macMinBE)
{
...
uint8_t
BleCsmaCa::GetMacMinBE (void) const
{
  NS_LOG_FUNCTION (this);
  return m_macMinBE;
}
...
void
BleCsmaCa::SetMacMaxBE (uint8_t macMaxBE)
{
...
uint8_t
BleCsmaCa::GetMacMaxBE (void) const
{
  NS_LOG_FUNCTION (this);
  return m_macMaxBE;
}
...
void
BleCsmaCa::SetMacMaxCSMABackoffs (uint8_t macMaxCSMABackoffs)
{
...
uint8_t
BleCsmaCa::GetMacMaxCSMABackoffs (void) const
{
  NS_LOG_FUNCTION (this);
...
void
BleCsmaCa::SetUnitBackoffPeriod (uint64_t unitBackoffPeriod)
{
...
uint64_t
BleCsmaCa::GetUnitBackoffPeriod (void) const
{
  NS_LOG_FUNCTION (this);
...
Time
BleCsmaCa::GetTimeToNextSlot (void) const
{
  NS_LOG_FUNCTION (this);
  ...
  if (IsSlottedCsmaCa ())
    {
      ...
      m_randomBackoffEvent = Simulator::Schedule (backoffBoundary, &BleCsmaCa::RandomBackoffDelay, this);
    }
  else
    {
      m_BE = m_macMinBE;
      m_randomBackoffEvent = Simulator::ScheduleNow (&BleCsmaCa::RandomBackoffDelay, this);
    }
  /*
  ...
  if (IsUnSlottedCsmaCa ())
    {
      ...
      m_requestCcaEvent = Simulator::Schedule (randomBackoff, &BleCsmaCa::RequestCCA, this);
    }
  else
    {
      NS_LOG_LOGIC ("Slotted:  proceeding after backoff of " << randomBackoff.GetMicroSeconds () << " us");
      m_canProceedEvent = Simulator::Schedule (randomBackoff, &BleCsmaCa::CanProceed, this);
    }
}
  ...
  if (canProceed)
    {
      ...
      Time backoffBoundary = GetTimeToNextSlot ();
      m_requestCcaEvent = Simulator::Schedule (backoffBoundary, &BleCsmaCa::RequestCCA, this);
    }
  else
    {
      Time nextCap = Seconds (0);
      m_randomBackoffEvent = Simulator::Schedule (nextCap, &BleCsmaCa::RandomBackoffDelay, this);
    }
}

void
BleCsmaCa::RequestCCA ()
{
  NS_LOG_FUNCTION (this);
 ...
 */
void
BleCsmaCa::PlmeCcaConfirm (BlePhyEnumeration status)
{
              ...
              if (m_CW == 0)
                {
                  ...
                  if (!m_bleMacStateCallback.IsNull ())
                    {
                      NS_LOG_LOGIC ("Notifying MAC of idle channel");
                      m_bleMacStateCallback (CHANNEL_IDLE);
                    }
                }
              else
                {
                  NS_LOG_LOGIC ("Perform CCA again, m_CW = " << m_CW);
                  m_requestCcaEvent = Simulator::ScheduleNow (&BleCsmaCa::RequestCCA, this); // Perform CCA again
                }
            }
          else
            {
      ...
      else
        {
          ...
          if (m_NB > m_macMaxCSMABackoffs)
              ...
              if (!m_bleMacStateCallback.IsNull ())
                {
                  ...
                  m_bleMacStateCallback (CHANNEL_ACCESS_FAILURE);
                }
              return;
            }
          else
            {
              ...
              m_randomBackoffEvent = Simulator::ScheduleNow (&BleCsmaCa::RandomBackoffDelay, this); //Perform another backoff (step 2)
            }
        }
    }
}
...
void
BleCsmaCa::SetBleMacStateCallback (BleMacStateCallback c)
{
  NS_LOG_FUNCTION (this);
  m_bleMacStateCallback = c;
}

int64_t
BleCsmaCa::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
