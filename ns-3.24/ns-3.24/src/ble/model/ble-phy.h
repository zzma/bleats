...
class SpectrumValue;
class BleErrorModel;
struct BleSpectrumSignalParameters;
class MobilityModel;
...
/**
 * \ingroup ble
 * TracedValue callback signature for BlePhyEnumeration.
 *
 * \param [in] oldValue original value of the traced variable
 * \param [in] newValue new value of the traced variable
 */
  typedef void (* BlePhyEnumeration)(BlePhyEnumeration oldValue,
                                        BlePhyEnumeration newValue);
}  // namespace TracedValueCallback
 ...
 */
typedef Callback< void, BlePhyEnumeration > PdDataConfirmCallback;
 ...
 */
typedef Callback< void, BlePhyEnumeration > PlmeCcaConfirmCallback;
 ...
 */
typedef Callback< void, BlePhyEnumeration,uint8_t > PlmeEdConfirmCallback;
 ...
 */
typedef Callback< void, BlePhyEnumeration,
                  BlePibAttributeIdentifier,
                  BlePhyPibAttributes* > PlmeGetAttributeConfirmCallback;
 ...
 */
typedef Callback< void, BlePhyEnumeration > PlmeSetTRXStateConfirmCallback;
 ...
 */
typedef Callback< void, BlePhyEnumeration,
                  BlePibAttributeIdentifier > PlmeSetAttributeConfirmCallback;

/**
 * \ingroup ble
 *
 * Make BlePhy a SpectrumPhy so we can enable the eventual modeling of
 * device interference
 */
...
public:
   ...
   */
  void PlmeGetAttributeRequest (BlePibAttributeIdentifier id);
   ...
   */
  void PlmeSetTRXStateRequest (BlePhyEnumeration state);
   ...
   */
  void PlmeSetAttributeRequest (BlePibAttributeIdentifier id, BlePhyPibAttributes* attribute);
   ...
   *
   * @param e pointer to BleErrorModel to use
   */
  void SetErrorModel (Ptr<BleErrorModel> e);

  /**
   * get the error model in use
   *
   * @return pointer to BleErrorModel in use
   */
  Ptr<BleErrorModel> GetErrorModel (void) const;
   ...
   * \deprecated The BlePhyEnumeration state is now accessible as the
   * TracedValue \c TrxStateValue.  The \c TrxState TracedCallback will
   * be removed in a future release.
   */
  typedef void (* StateTracedCallback)
    (Time time, BlePhyEnumeration oldState, BlePhyEnumeration newState);
...
protected:
   ...
   */
  static const BlePhyDataAndSymbolRates dataSymbolRates[7];
  /**
   * The preamble, SFD, and PHR lengths in symbols for the different PHY options.
   * See Table 19 and Table 20 in section 6.3 IEEE 802.15.4-2006
   */
  static const BlePhyPpduHeaderSymbolNumber ppduHeaderSymbolNumbers[7];
...
private:
   ...
   */
  void ChangeTrxState (BlePhyEnumeration newState);
   ...
   */
  BlePhyOption GetMyPhyOption (void);
   ...
   */
  void CancelEd (BlePhyEnumeration state);
   ...
   * be removed in a future release.
   */
  TracedCallback<Time, BlePhyEnumeration, BlePhyEnumeration> m_trxStateLogger;
   ...
   */
  Ptr<BleErrorModel> m_errorModel;

  /**
   * The current PHY PIB attributes.
   */
  BlePhyPibAttributes m_phyPIBAttributes;

  // State variables
  /**
   * The current transceiver state.
   */
  TracedValue<BlePhyEnumeration> m_trxState;

  /**
   * The next pending state to applied after the current action of the PHY is
   * completed.
   */
  BlePhyEnumeration m_trxStatePending;
   ...
   * due to interference.
   */
  std::pair<Ptr<BleSpectrumSignalParameters>, bool>  m_currentRxPacket;
