...
NS_LOG_COMPONENT_DEFINE ("ble-ack-test");

class BleAckTestCase : public TestCase
{
public:
  BleAckTestCase ();

  static void DataIndication (BleAckTestCase *testCase, Ptr<BleNetDevice> dev, McpsDataIndicationParams params, Ptr<Packet> p);
  static void DataConfirm (BleAckTestCase *testCase, Ptr<BleNetDevice> dev, McpsDataConfirmParams params);
...
void
BleAckTestCase::DataIndication (BleAckTestCase *testCase, Ptr<BleNetDevice> dev, McpsDataIndicationParams params, Ptr<Packet> p)
{
...
void
BleAckTestCase::DataConfirm (BleAckTestCase *testCase, Ptr<BleNetDevice> dev, McpsDataConfirmParams params)
{
...
BleAckTestCase::DoRun (void)
{
  ...
  cb0 = MakeBoundCallback (&BleAckTestCase::DataConfirm, this, dev0);
  dev0->GetMac ()->SetMcpsDataConfirmCallback (cb0);

  McpsDataIndicationCallback cb1;
  cb1 = MakeBoundCallback (&BleAckTestCase::DataIndication, this, dev0);
  dev0->GetMac ()->SetMcpsDataIndicationCallback (cb1);

  McpsDataConfirmCallback cb2;
  cb2 = MakeBoundCallback (&BleAckTestCase::DataConfirm, this, dev1);
  dev1->GetMac ()->SetMcpsDataConfirmCallback (cb2);

  McpsDataIndicationCallback cb3;
  cb3 = MakeBoundCallback (&BleAckTestCase::DataIndication, this, dev1);
...
class BleAckTestSuite : public TestSuite
{
public:
  BleAckTestSuite ();
};

BleAckTestSuite::BleAckTestSuite ()
  : TestSuite ("ble-ack", UNIT)
{
  AddTestCase (new BleAckTestCase, TestCase::QUICK);
}

static BleAckTestSuite g_bleAckTestSuite;
