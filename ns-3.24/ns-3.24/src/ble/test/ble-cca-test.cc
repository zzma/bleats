...
NS_LOG_COMPONENT_DEFINE ("ble-clear-channel-assessment-test");

class BleCcaTestCase : public TestCase
{
public:
  BleCcaTestCase ();

private:
  static void PlmeCcaConfirm (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, BlePhyEnumeration status);
  static void PhyTxBegin (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet);
  static void PhyTxEnd (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet);
  static void PhyRxBegin (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet);
  static void PhyRxEnd (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet, double sinr);
  static void PhyRxDrop (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet);

  virtual void DoRun (void);

  BlePhyEnumeration m_status;

};

BleCcaTestCase::BleCcaTestCase ()
...
void
BleCcaTestCase::PlmeCcaConfirm (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, BlePhyEnumeration status)
{
  std::cout << std::setiosflags (std::ios::fixed) << std::setprecision (9) << "[" << Simulator::Now ().GetSeconds () << "] " << device->GetMac ()->GetShortAddress () << " PlmeCcaConfirm: " << BleHelper::BlePhyEnumerationPrinter (status) << std::endl;

  testcase->m_status = status;
}

void
BleCcaTestCase::PhyTxBegin (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet)
{
...
void
BleCcaTestCase::PhyTxEnd (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet)
{
  std::ostringstream os;
  packet->Print (os);
...
void
BleCcaTestCase::PhyRxBegin (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet)
{
  std::ostringstream os;
  packet->Print (os);
...
void
BleCcaTestCase::PhyRxEnd (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet, double sinr)
{
  std::ostringstream os;
  packet->Print (os);
...
void BleCcaTestCase::PhyRxDrop (BleCcaTestCase *testcase, Ptr<BleNetDevice> device, Ptr<const Packet> packet)
{
  std::ostringstream os;
  packet->Print (os);
...
BleCcaTestCase::DoRun (void)
{
  ...
  dev1->GetPhy ()->SetPlmeCcaConfirmCallback (MakeBoundCallback (&BleCcaTestCase::PlmeCcaConfirm, this, dev1));
  ...
  dev0->GetPhy ()->TraceConnectWithoutContext ("PhyTxBegin", MakeBoundCallback (&BleCcaTestCase::PhyTxBegin, this, dev0));
  dev0->GetPhy ()->TraceConnectWithoutContext ("PhyTxEnd", MakeBoundCallback (&BleCcaTestCase::PhyTxEnd, this, dev0));
  dev2->GetPhy ()->TraceConnectWithoutContext ("PhyTxBegin", MakeBoundCallback (&BleCcaTestCase::PhyTxBegin, this, dev2));
  dev2->GetPhy ()->TraceConnectWithoutContext ("PhyTxEnd", MakeBoundCallback (&BleCcaTestCase::PhyTxEnd, this, dev2));
  dev1->GetPhy ()->TraceConnectWithoutContext ("PhyRxBegin", MakeBoundCallback (&BleCcaTestCase::PhyRxBegin, this, dev1));
  dev1->GetPhy ()->TraceConnectWithoutContext ("PhyRxEnd", MakeBoundCallback (&BleCcaTestCase::PhyRxEnd, this, dev1));
  dev1->GetPhy ()->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&BleCcaTestCase::PhyRxDrop, this, dev1));
  ...
  Simulator::ScheduleNow (&BleMac::McpsDataRequest, dev0->GetMac (), params0, p0);
  ...
  Simulator::ScheduleNow (&BleMac::McpsDataRequest, dev2->GetMac (), params1, p1);
...
class BleCcaTestSuite : public TestSuite
{
public:
  BleCcaTestSuite ();
};

BleCcaTestSuite::BleCcaTestSuite ()
  : TestSuite ("ble-clear-channel-assessment", UNIT)
{
  AddTestCase (new BleCcaTestCase, TestCase::QUICK);
}

static BleCcaTestSuite g_bleCcaTestSuite;
